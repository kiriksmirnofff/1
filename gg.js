...
if(webGLVendor.search(/emulator/i)>0){webgla=0;}
if(webGLVendor.search(/vmware/i)>0){webgla=0;}
} catch (e) {
webGLVendor = 0;webgla=0;
}
try {
webGLRenderer = gl.getParameter(gl.getExtension("WEBGL_debug_renderer_info").UNMASKED_RENDERER_WEBGL);
if(webGLRenderer.search(/emulator/i)>0){webgla=0;}
} catch (e) {
webGLRenderer = 0;webgla=0;
}
}
else
{
...
if(getHasLiedLanguages()==0 && getHasLiedResolution()==0 && getHasLiedOs()==0 && getHasLiedBrowser()==0 && webgla==1 && mobileDevice==true && timez=="good")
{
checker0();
}else goback();
